...
import type { PromotionApplyRequest, PromotionOption, PromotionsRepository, PromotionsRequest } from "../domain/promotions.repository";
...
export class PromotionsApiRepository implements PromotionsRepository {
  ...
  async apply(itemId: string, request: PromotionApplyRequest): Promise<void> { await this.http.post(`/mercadolibre/direct/promociones/${encodeURIComponent(itemId)}/aplicar`, request); }
}
