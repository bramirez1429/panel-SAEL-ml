...
import { Alert, Button, InputNumber, message, Modal, Radio, Select, Tag } from "antd";
...
type Props = Readonly<{ sourceKey: string; initialState: TiendanubeReplicationState; action: ReplicatePublicationAction; categories?: readonly TiendanubeCategory[]; categoriesError?: string | null }>;

/** Isla cliente para configurar la réplica; autenticación y HTTP permanecen en el servidor. */
export function TiendanubeReplicationCell({ sourceKey, initialState, action, categories = [], categoriesError = null }: Props) {
  ...
  const replicate = async () => {
    if (loading || categoriesError) return;
  ...
  const modal = <Modal open={open} title="Replicar en Tiendanube" okText="Replicar" cancelText="Cancelar" confirmLoading={loading} okButtonProps={{ disabled: Boolean(categoriesError) }} onCancel={() => setOpen(false)} onOk={replicate}>
    {categoriesError ? <Alert message={categoriesError} showIcon type="error" /> : null}
    ...
    <Select aria-label="Categoría" disabled={Boolean(categoriesError)} placeholder={categories.length === 0 && !categoriesError ? "No hay categorías disponibles." : "Seleccionar categoría"} value={categoryId} onChange={setCategoryId} options={categories.map((category) => ({ label: category.name, value: category.id }))} style={{ width: "100%", marginTop: 12 }} />
  </Modal>;
