...
describe("TiendanubeReplicationCell", () => {
  ...
  it("muestra Replicar TN activo cuando no está replicada", () => {
    render(<TiendanubeReplicationCell action={vi.fn()} initialState={state} sourceId={uuid} />);
  ...
  it("muestra Procesando deshabilitado mientras el estado está pendiente", () => {
    render(<TiendanubeReplicationCell action={vi.fn()} initialState={{ ...state, status: "PENDING" }} sourceId={uuid} />);
    expect(screen.getByRole("button", { name: "Procesando..." })).toBeDisabled();
  });

  it("muestra Reintentar activo cuando la replicación falló", () => {
    render(<TiendanubeReplicationCell action={vi.fn()} initialState={{ ...state, status: "FAILED" }} sourceId={uuid} />);
    expect(screen.getByRole("button", { name: "Reintentar" })).toBeEnabled();
  });

  it("muestra Replicado y deja volver a replicar activo cuando está completada", () => {
    const { unmount } = render(<TiendanubeReplicationCell action={vi.fn()} initialState={{ ...state, status: "COMPLETED" }} sourceId={uuid} />);
    expect(screen.getByText("✓ Replicado")).toBeInTheDocument();
    unmount();
    render(<TiendanubeRereplicationCell action={vi.fn()} initialState={{ ...state, status: "COMPLETED" }} sourceId={uuid} />);
  ...
  it("deshabilita la acción sólo durante la request y refresca al confirmar", async () => {
    ...
    render(<TiendanubeReplicationCell action={action} initialState={state} sourceId={uuid} />);
    ...
    expect(action).toHaveBeenCalledWith(uuid);
  });
