...
export class PublicationEditApiRepository implements PublicationEditRepository {
  ...
  async getSku(target: PublicationEditTarget): Promise<string | null> {
    const path = target.type === "family" ? `/mercadolibre/direct/edicion/nueva/${encodeURIComponent(target.familyId)}/items/${encodeURIComponent(target.itemId)}/sku` : `/mercadolibre/direct/edicion/clasica/${encodeURIComponent(target.itemId)}/sku`;
    ...
    if (target.type === "legacy" && target.variationId !== null) {
      return response.variations?.find((variation) => String(variation.variationId) === String(target.variationId))?.sku ?? null;
    }
    return response.sku ?? null;
  }

  async updatePrice(target: PublicationEditTarget, price: number): Promise<void> {
    const path = target.type === "family" ? `/mercadolibre/direct/edicion/nueva/${encodeURIComponent(target.familyId)}/items/${encodeURIComponent(target.itemId)}` : `/mercadolibre/direct/edicion/clasica/${encodeURIComponent(target.itemId)}`;
    await this.httpClient.patch(path, { price });
  }

  async updateStock(target: PublicationEditTarget, quantity: number): Promise<void> {
    const path = target.type === "family" ? `/mercadolibre/direct/edicion/nueva/${encodeURIComponent(target.familyId)}/items/${encodeURIComponent(target.itemId)}/stock` : `/mercadolibre/direct/edicion/clasica/${encodeURIComponent(target.itemId)}/stock`;
    const body = target.type === "legacy" && target.variationId !== null ? { quantity, variationId: target.variationId } : { quantity };
    await this.httpClient.patch(path, body);
  }

  async updateSku(target: PublicationEditTarget, sku: string): Promise<void> {
    const path = target.type === "family" ? `/mercadolibre/direct/edicion/nueva/${encodeURIComponent(target.familyId)}/items/${encodeURIComponent(target.itemId)}/sku` : `/mercadolibre/direct/edicion/clasica/${encodeURIComponent(target.itemId)}/sku`;
    const body = target.type === "legacy" && target.variationId !== null ? { sku, variationId: target.variationId } : { sku };
    await this.httpClient.patch(path, body);
  }
  ...
  async deleteVariation(itemId: string, variationId: number): Promise<void> {
    await this.httpClient.delete(`/mercadolibre/direct/edicion/items/${encodeURIComponent(itemId)}/variations/${encodeURIComponent(String(variationId))}`);
  }
}
