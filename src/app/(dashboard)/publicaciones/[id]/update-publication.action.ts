...
import { createUpdatePublicationCommand } from "@/modules/publications/publications.composition.server";
...
export type UpdatePublicationStatusAction = (
  input: Readonly<{
...
export async function updatePublicationAction(input: UpdatePublicationInput): Promise<UpdatePublicationActionResult> {
  try {
    ...
    for (let attempt = 0; attempt < 3; attempt += 1) {
      ...
      detail = freshDetail;
      const confirmedSku = changedFields.sku === undefined ? undefined : await command.getSku(input.target);
    ...
    return {
      ok: true,
      confirmed: detail ? getConfirmedValues(detail, input) : {},
    };
  } catch (error: unknown) {
...
export async function updatePublicationStatusAction(
  ...
  try {
    ...
    for (let attempt = 0; attempt < 3; attempt += 1) {
      ...
      const variant = input.target.type === "family" ? detail.variants.find((item) => item.itemId === input.target.itemId) : null;
    ...
    return {
      ok: false,
      message: `El backend todavía no confirmó el estado ${input.status}.`,
    };
  } catch (error: unknown) {
    return {
      ok: false,
      message: error instanceof AppError ? error.message : "No se pudo actualizar el estado.",
    };
  }
}

export async function deletePublicationVariationAction(
  input: Readonly<{
    publicationId: string;
...
): Promise<DeletePublicationVariationActionResult> {
  if (input.publicationType !== "LEGACY") {
    return unsupportedUserProductDelete();
  }
  if (!/^MLA\d+$/.test(input.itemId) || !Number.isSafeInteger(input.variationId) || input.variationId < 0) {
    return {
      ok: false,
      message: "Los identificadores de la variante no son válidos.",
    };
  }

  try {
    await createUpdatePublicationCommand().deleteVariation(input.itemId, input.variationId);
    revalidatePath("/publicaciones");
    revalidatePath(`/publicaciones/${input.publicationId}`);
    return { ok: true };
  } catch (error: unknown) {
    const code = backendErrorCode(error);
    if (code === "USER_PRODUCT_VARIATION_DELETE_NOT_SUPPORTED") {
      return unsupportedUserProductDelete();
    }
    return {
      ok: false,
      code,
      message: error instanceof AppError ? error.message : "No se pudo eliminar la variante.",
    };
  }
}

function unsupportedUserProductDelete(): DeletePublicationVariationActionResult {
  return {
    ok: false,
    code: "USER_PRODUCT_VARIATION_DELETE_NOT_SUPPORTED",
    message: "Esta publicación utiliza el nuevo modelo de variantes de Mercado Libre y todavía no puede eliminarse mediante esta operación.",
  };
}
...
function backendErrorCode(error: unknown): string | undefined {
  ...
  const code = (error.responseBody as Record<string, unknown>).code;
...
function getConfirmedValues(
  ...
  const variant = input.target.type === "family" ? detail.variants.find((item) => item.itemId === input.target.itemId) : variationId === null ? null : detail.variants.find((item) => item.id === String(variationId));
  return {
    sku: confirmedSku === undefined ? (variant?.sku ?? (variant ? null : getAttributeValue(detail, "SELLER_SKU"))) : confirmedSku,
...
function matchesChangedFields(
  ...
  return (changed.sku === undefined || confirmed.sku === changed.sku) && (changed.price === undefined || confirmed.price === changed.price) && (changed.stock === undefined || confirmed.stock === changed.stock);
}
