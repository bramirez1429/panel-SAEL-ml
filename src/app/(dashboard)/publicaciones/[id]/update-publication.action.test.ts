...
describe("updatePublicationAction", () => {
  it("confirma el SKU mediante el endpoint específico", async () => {
    ...
    mocks.refresh.mockResolvedValue({
      ...
      thumbnailUrl: null,
      permalink: null,
      price: { from: 11, to: 11, currency: null },
      stock: 3,
      sold: 0,
      attributes: [],
      group: {
    ...
    await expect(updatePublicationAction(input)).resolves.toMatchObject({
      ok: true,
      confirmed: { sku: "NEW" },
    });
  ...
  it("confirma la mutación leyendo nuevamente el detalle", async () => {
    ...
    await expect(updatePublicationAction(input)).resolves.toMatchObject({
      ok: true,
      confirmed: { sku: "NEW", price: 11, stock: 3 },
    });
  ...
  it("reintenta cuando Mercado Libre propaga el valor con demora", async () => {
    ...
    const stale = {
      ...
      thumbnailUrl: null,
      permalink: null,
      price: { from: 10, to: 10, currency: null },
      stock: 2,
      sold: 0,
      attributes: [],
      group: {
  ...
  it("reintenta el estado hasta confirmar paused", async () => {
    ...
    ).resolves.toEqual({ ok: true, confirmed: "paused" });
    expect(mocks.refresh).toHaveBeenCalledTimes(2);
  });
});

describe("deletePublicationVariationAction", () => {
  it("elimina una variante legacy por itemId y variationId", async () => {
    mocks.deleteVariation.mockResolvedValue(undefined);
    await expect(
      deletePublicationVariationAction({
        publicationId: "MLA123",
        publicationType: "LEGACY",
        itemId: "MLA123",
        variationId: 456,
      }),
    ).resolves.toEqual({ ok: true });
    expect(mocks.deleteVariation).toHaveBeenCalledWith("MLA123", 456);
  });

  it("no trata USER_PRODUCT como una variante legacy", async () => {
    mocks.deleteVariation.mockClear();
    await expect(
      deletePublicationVariationAction({
        publicationId: "family:F1",
    ...
    expect(mocks.deleteVariation).not.toHaveBeenCalled();
  });
});
